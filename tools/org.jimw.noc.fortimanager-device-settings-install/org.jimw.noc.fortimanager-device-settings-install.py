...
"""
FortiManager Device Settings Install

Push pending device-scope settings via exec /securityconsole/install/device.
...
async def execute(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    dev_rev_comments = params.get("dev_rev_comments") or "Device settings installed via FortiManager AI SDK"
    ...
    for s in scope:
        ...
        norm_scope.append({"name": s["name"], "vdom": s.get("vdom") or "root"})
    ...
    try:
        ...
        if fmg_status.get("code") != 0:
            return {
                "success": False, "adom": adom,
                "error": f"FMG exec error: {fmg_status}",
            }

        task_id = (result.get("data") or {}).get("task")
        if task_id is None:
            return {"success": False, "adom": adom, "error": "FMG returned no task ID"}
        task_id = int(task_id)

        out: Dict[str, Any] = {
            "success": True, "task_id": task_id, "adom": adom,
        ...
        logger.exception("device-settings-install failed")
...
if __name__ == "__main__":
    ...
    dev = sys.argv[2] if len(sys.argv) > 2 else "howard-sdwan-spoke-1"
